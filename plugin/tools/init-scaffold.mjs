#!/usr/bin/env node
/**
 * /workflow:init 的执行体 —— 把插件 templates/ 释放到目标项目,并补根 CLAUDE.md / AGENTS.md。
 *
...
/** 根 CLAUDE.md / AGENTS.md 指向的三件项目专属文件(通用规则由插件注入,不在这里登记)。 */
export const IMPORT_LINES = ['@.spec/AGENTS.md', '@.spec/knowledge/README.md', '@.spec/rules/system.md']

/** 根 CLAUDE.md:只有三行 @import,Claude 会展开。 */
export const CLAUDE_MD = `${IMPORT_LINES.join('\n')}\n`

/** 根 AGENTS.md:同一组指针。Codex 不展开 @import,须主动 Read。 */
export const AGENTS_MD = `Codex 须主动 Read 这三份。
 ...
 *   imports = 追加进已存在 CLAUDE.md 的 @import 行(新建 CLAUDE.md 时为空,它已包含全部三行;AGENTS.md 不补行)
 */
export function initScaffold({ pluginRoot, target, force = false }) {
  ...
  place('AGENTS.md', AGENTS_MD)

  // 已存在的 CLAUDE.md 未被覆盖时,只补缺失的 @import 行——幂等,不动用户已有内容。
  // AGENTS.md 已存在则跳过,不补行、不改写(Codex 不把 @import 当 include)。
  const imports = []
