...
它按 `workflow-ops` 的凭证顺序取值：环境变量 `WORKFLOW_API_BASE` + `WORKFLOW_TOKEN` → 当前目录向上最近的 `.workflow` 标记 → `~/.config/workflow/config.toml` 对应 profile；**绝不回落全局 `current_profile`**——当前目录没有 `.workflow` 就什么都不拉，先转 `workflow-init` 绑定。索引落在 `$XDG_CACHE_HOME/workflow/index/<host>.json`（缺 XDG 时 `~/.cache`），不进仓、不进插件根。首次或超过 24 小时走全量（按 Room 枚举需求），否则走增量；联不上就沿用旧快照并标「离线沿用」，退出码恒 0。
