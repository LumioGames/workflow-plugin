...
> **什么时候才值得派 worker**——三条满足一条即可，都不满足就自己直接做：
> ① 多张互不依赖的单可并行；② 改动大到撑爆编排上下文；③ 需要隔离的干净实现环境。
> 判据模糊时按「省不省上下文」判：派一个 worker 的固定开销（写 brief、收 report、查冲突、合入）
> 不比自己做小，只有一次省下的上下文大于这份开销才划算。
