...
用户从询问转为要实际执行操作（建需求、记 bug、改状态……）时，**转 workflow-ops 技能**；需要先接入或修连接问题时转 workflow-init。用户不是在问用法，而是要**向平台方报问题、提体验或功能建议**（平台缺陷、太卡太慢、希望加功能）时，转 workflow-feedback。本技能只答疑，不执行写操作。
