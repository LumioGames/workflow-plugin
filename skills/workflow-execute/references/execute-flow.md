...
- **先加载再动手**：改动规模决定读多深——**小**（≤1 个文件、逻辑独立）只读直接相关的知识文档 + 被改源文件；**中**（2–5 个文件、跨模块）经 `knowledge/README.md` 导航读全部相关文档与技能，先列改动清单再逐项做；**大**（多模块、多步骤、有依赖）停下报告，建议经 `workflow-planning` 拆单。低估规模是最常见的失误。读完就用，不复述到回复里。
...
目标仓库里的开发/制作按卡内要求与所在环境规则执行；插件不管辖怎么写代码，但**并行纪律**按下面执行。宿主支持子 Agent（Task / 子任务机制）时，按第四节确认过的拆分**尽可能并行**，加快交付：
...
- **汇总由主执行者负责**：合并产出、消解冲突后整体验证由主执行者自己做——子 Agent 声称的「完成」不算证据，要么收集其实跑输出、要么自己重跑、要么如实写「未执行」（G7；推分支前不要求跑任何命令，ADR-087）。
