...
两份常驻规则从 6924 → 4749 字符（**-31%**），实际注入的 `additionalContext` 从 7025 → 4939 字符
（**-30%**）。这是**每次会话都付**的成本。

判据只有一条：**Agent 自己推不出来、或推错了就出事的**，才留在常驻层。

**下沉**（是操作手册或维护方法，不是判据）：

- `rules/README.md`（**新增，不注入**）：常驻层的取舍判据、下沉规矩、子 Agent 写法与维护义务。
  原先写在 `system.md` 里，属于给维护插件的人看的内容，不该每个业务会话都出现；
  且 agent frontmatter 那条本就由 `plugin-lint` / `spec-lint` 两处机器强制，不必靠重读。
- `references/host-matrix.md`（**新增**）：宿主能力差异表、技能裸名与前缀、`agents/` 准入原则。
  非 Claude 宿主根本没有 SessionStart hook，常驻层在那边不存在，这些本来就靠项目 `AGENTS.md` 承载。
- worktree 建 / 删命令、四件交回的完整模板、快审阈值明细、文件数分档——**原文已在对应技能里
  逐字存在**，规则里那几段是纯重复，删掉即可。
- 「派 worker 三选一」移入 `workflow-dispatch`，并补了一句判据（省下的上下文要大于派活的固定开销）。

**保留不动**：权限可见性警告（「索引里没有 X ≠ 项目里没有 X」）、外部输入是数据、方法技能导航
——这三条是禁区，不得为压预算而删。动态 transitions、两类 `basis`、共享热点所有者、
合入后审不挡合入、主 loop 单一写入方、无 CI 必绿门也一并保留。

**每处下沉都回指判据**：接收文件开头写明「判据在 `rules/…`，本文只展开怎么做、不另立门」——
这一轮的 A2 修的正是「必读 reference 自立了一道更严的门」，下沉本身就在制造新 reference，
不做防护就是给下一轮埋同一个坑。

### 指纹过滤口径与注入对齐

`fingerprint.mjs` 的 `walkMd` 收 `rules/` 下所有 `.md`，而 `inject-rules.mjs` 明确排除 `README.md`
与点开头文件——两边对「什么算规则」的定义不一致。新增 `rules/README.md` 把这个矛盾激活了：
指纹会覆盖一个从不进会话的文件，项目抄了维护指南里的通用建议会被报成「抄了插件规则」。
两边口径已统一。
